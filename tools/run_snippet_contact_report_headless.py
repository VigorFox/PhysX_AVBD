#!/usr/bin/env python3
"""Run the SnippetContactReport failure-first or acceptance matrix headlessly."""
...
EXECUTABLE = "SnippetContactReport_64.exe"
...
def run_one(
    ...
    print(
        f"[CONTACT_REPORT_RUN] name={spec.name} "
    ...
    for error in errors:
        print(
            f"[CONTACT_REPORT_RUN_ERROR] name={spec.name} error={error}"
        )
...
def main() -> int:
    ...
    if not executable.is_file():
        print(
            "[CONTACT_REPORT_RUNNER_ERROR] "
            f"missing executable: {executable}"
        )
        return 2
    if args.timeout <= 0.0:
        print(
            "[CONTACT_REPORT_RUNNER_ERROR] --timeout must be positive"
        )
        return 2
    ...
    print(
        f"[CONTACT_REPORT_MATRIX] mode={args.mode} "
