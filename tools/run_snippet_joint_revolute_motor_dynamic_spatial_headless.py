#!/usr/bin/env python3
"""Gate dynamic-pair off-center plus off-principal revolute motor semantics."""
...
CASE = "revolute-motor-dynamic-spatial"
FRAMES = 360
DT = 1.0 / 60.0
...
def run_one(spec: RunSpec, bin_dir: Path, timeout: float) -> bool:
    ...
    for key, expected in {
        "case": CASE,
        "topology": "dynamic-dynamic-spatial",
        "actorOrderValid": "1",
        "driveEnabled": "1",
        "freeSpinDisabled": "1",
        "targetVelocity": "2",
    ...
    if all(value is not None for value in metrics.values()):
        fixture_is_independent = (
            ...
            and metrics["initialPerpendicularLeverArmA"] >= 0.5
        ...
        if not fixture_is_independent:
            errors.append(
                "fixture lacks independent off-principal and lever-arm witnesses"
            )
        if spec.expected_status == "PASS" and not physical_pass:
            errors.append("PASS status lacks dynamic spatial semantics")
        if spec.expected_status == "FAIL" and physical_pass:
            errors.append("FAIL status lacks an independent red metric")

    print(
        "[REVOLUTE_MOTOR_DYNAMIC_SPATIAL_RUN] "
        ...
        f"responseA="
        f"{fixture.get('initialOffPrincipalResponseA', 'MISSING')} "
        f"responseB="
        f"{fixture.get('initialOffPrincipalResponseB', 'MISSING')} "
        f"swing="
    ...
    for error in errors:
        print(
            "[REVOLUTE_MOTOR_DYNAMIC_SPATIAL_ERROR] "
            f"solver={spec.solver} execution={spec.execution} "
            f"error={error}"
        )
    return not errors
...
def main() -> int:
    ...
    print(
        "[REVOLUTE_MOTOR_DYNAMIC_SPATIAL_SUMMARY] "
