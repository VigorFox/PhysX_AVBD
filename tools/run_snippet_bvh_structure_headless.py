#!/usr/bin/env python3
"""Run the SnippetBVHStructure compound-query gate headlessly."""
...
EXECUTABLE = "SnippetBVHStructure_64.exe"
FRAMES = 60
...
def run_one(
    ...
    for key in ("maxSpeed", "maxAngularSpeed", "maxDisplacement"):
        try:
            value = float(fields[key])
            if not math.isfinite(value):
                errors.append(f"{key}={value}, expected finite")
            if key == "maxDisplacement" and value <= 0:
                errors.append(f"{key}={value}, expected > 0")
            if value >= 50:
                errors.append(f"{key}={value}, expected < 50")
    ...
    print(
        f"[BVH_STRUCTURE_RUN] name={spec.name} "
    ...
    for error in errors:
        print(f"[BVH_STRUCTURE_RUN_ERROR] name={spec.name} error={error}")
...
def compare_repeats(results: dict[str, dict[str, str]]) -> bool:
    ...
    for solver, execution in (
        ...
        print(
            f"[BVH_STRUCTURE_REPEAT] pair={solver}-{execution} "
...
def main() -> int:
    ...
    if not (bin_dir / EXECUTABLE).is_file():
        print(
            f"[BVH_STRUCTURE_RUNNER_ERROR] missing executable: "
            f"{bin_dir / EXECUTABLE}"
        )
        return 2
    if args.timeout <= 0:
        print("[BVH_STRUCTURE_RUNNER_ERROR] --timeout must be positive")
        return 2
    if args.frames <= 0:
        print("[BVH_STRUCTURE_RUNNER_ERROR] --frames must be positive")
        return 2
    ...
    print(
        f"[BVH_STRUCTURE_MATRIX] mode={args.mode} "
