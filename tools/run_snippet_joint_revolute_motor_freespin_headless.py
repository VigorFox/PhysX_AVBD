#!/usr/bin/env python3
"""Gate native revolute free-spin motor semantics headlessly."""
...
FRAMES = 360
DT = 1.0 / 60.0
CASE_NAME = "revolute-motor-freespin"
TOPOLOGY = "world-dynamic"
EXPECTED_DYNAMIC_ACTORS = "1"
REQUIRE_MOMENTUM = False
OUTPUT_TAG = "REVOLUTE_MOTOR_FREESPIN"
...
def run_one(spec: RunSpec, bin_dir: Path, timeout: float) -> bool:
    ...
    prefixes = {
        "gate": "[AVBD_GATE] ",
        "fixture": "[PROBE] [SnippetJointRevoluteMotorFreeSpin] ",
        "cleanup": (
    ...
    if spec.expected_status == "FAIL" and gate.get("reason") not in {
        "revolute_motor_freespin_acceleration",
        "revolute_motor_freespin_braking",
        "revolute_motor_freespin_momentum",
    }:
        errors.append(f"unexpected failure reason: {gate.get('reason')!r}")

    metric_keys = (
        "boostVelocityReadback",
        "preBoostFinalVelocity",
        "maximumLatePreBoostError",
        "finalVelocity",
        "minimumPostBoostVelocity",
        "maximumPostBoostVelocityDrop",
        "maximumAngularMomentumDrift",
        "maximumAnchorError",
    ...
    if all(value is not None for value in metrics.values()):
        physical_pass = (
            abs(metrics["boostVelocityReadback"] - 5.0) <= 0.01
            and abs(metrics["preBoostFinalVelocity"] - 2.0) <= 0.05
            and metrics["maximumLatePreBoostError"] <= 0.05
            and metrics["finalVelocity"] >= 4.9
            and metrics["minimumPostBoostVelocity"] >= 4.9
            and metrics["maximumPostBoostVelocityDrop"] <= 0.1
            and (
                not REQUIRE_MOMENTUM
                or metrics["maximumAngularMomentumDrift"] <= 1.0e-3
            )
        ...
        if spec.expected_status == "PASS" and not physical_pass:
            errors.append("PASS status lacks independent free-spin semantics")
    ...
    print(
        ...
        f"preBoostVelocity="
        f"{fixture.get('preBoostFinalVelocity', 'MISSING')} "
        f"preBoostError="
        f"{fixture.get('maximumLatePreBoostError', 'MISSING')} "
        f"boostReadback="
        f"{fixture.get('boostVelocityReadback', 'MISSING')} "
        f"minimumPostBoost="
        f"{fixture.get('minimumPostBoostVelocity', 'MISSING')} "
        f"finalVelocity={fixture.get('finalVelocity', 'MISSING')} "
        f"velocityDrop="
        f"{fixture.get('maximumPostBoostVelocityDrop', 'MISSING')} "
        f"momentum="
        f"{fixture.get('maximumAngularMomentumDrift', 'MISSING')} "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run native revolute free-spin ownership headlessly."
    )
    parser.add_argument(
        "--mode",
