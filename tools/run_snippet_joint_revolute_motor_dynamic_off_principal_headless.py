#!/usr/bin/env python3
"""Gate centered dynamic-pair off-principal revolute drive semantics."""
...
CASE = "revolute-motor-dynamic-off-principal"
FRAMES = 360
DT = 1.0 / 60.0
...
def run_one(spec: RunSpec, bin_dir: Path, timeout: float) -> bool:
    ...
    for key, expected in {
        "case": CASE,
        "topology": "dynamic-dynamic-centered",
        "actorOrderValid": "1",
        "driveEnabled": "1",
        "freeSpinDisabled": "1",
        "targetVelocity": "2",
    ...
    metric_keys = (
        ...
        "initialOffPrincipalResponseA",
        "initialOffPrincipalResponseB",
        "maximumLateRelativeSwingVelocity",
        "maximumInitialGeneralizedMomentumDrift",
        "maximumGeneralizedMomentumDrift",
        "maximumAnchorError",
    ...
    if all(value is not None for value in metrics.values()):
        fixture_is_independent = (
            metrics["initialOffPrincipalResponseA"] >= 0.05
            and metrics["initialOffPrincipalResponseB"] >= 0.05
        )
        physical_pass = (
            fixture_is_independent
    ...
    print(
        ...
        f"momentum="
        f"{fixture.get('maximumInitialGeneralizedMomentumDrift', 'MISSING')} "
        f"longMomentum="
        f"{fixture.get('maximumGeneralizedMomentumDrift', 'MISSING')} "
    ...
    for error in errors:
        print(
            "[REVOLUTE_MOTOR_DYNAMIC_OFF_PRINCIPAL_ERROR] "
            f"solver={spec.solver} execution={spec.execution} "
            f"error={error}"
        )
    if errors and gate.get("reason") == "initialization":
        print(
            "[REVOLUTE_MOTOR_DYNAMIC_OFF_PRINCIPAL_DIAGNOSTIC] "
            + combined.replace("\r", "").replace("\n", " | ")
        )
    return not errors
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run centered dynamic-pair off-principal revolute drive "
            "headlessly."
        )
    )
    parser.add_argument(
    ...
    print(
        "[REVOLUTE_MOTOR_DYNAMIC_OFF_PRINCIPAL_SUMMARY] "
