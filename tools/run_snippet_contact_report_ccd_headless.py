#!/usr/bin/env python3
"""Run the SnippetContactReportCCD failure-first matrix headlessly."""
...
EXECUTABLE = "SnippetContactReportCCD_64.exe"
...
def run_one(
    ...
    if mode == "acceptance":
        ...
        if fields.get("validation") != "GATED":
            errors.append(
                f"validation={fields.get('validation')!r}, "
                "expected 'GATED'"
            )
    ...
    print(
        f"[CONTACT_REPORT_CCD_RUN] name={spec.name} "
...
def main() -> int:
    ...
    if not executable.is_file():
        print(
            "[CONTACT_REPORT_CCD_RUNNER_ERROR] "
            f"missing executable: {executable}"
        )
        return 2
    if args.timeout <= 0.0:
        print(
            "[CONTACT_REPORT_CCD_RUNNER_ERROR] --timeout must be positive"
        )
        return 2
    ...
    print(
        f"[CONTACT_REPORT_CCD_MATRIX] mode={args.mode} "
