#!/usr/bin/env python3
"""Run the SnippetVehicleFourWheelDrive official command-cycle headless gate."""
...
EXECUTABLE = "SnippetVehicleFourWheelDrive_64.exe"
...
def run_one(
    ...
    required = {
        "schema": "1",
        "snippet": "SnippetVehicleFourWheelDrive",
        "solver": spec.solver,
        "case": "command-cycle",
    ...
    try:
        ...
        if int(fields["fourWheelDifferentialFrames"]) <= completed // 2:
            errors.append("four-wheel differential was not sustained")
        for key in (
            "driveFrames",
            "brakeFrames",
            "activeConstraintFrames",
        ...
        shift_expected = int(fields["automaticShiftExpected"])
        gear_changes = int(fields["gearChanges"])
        minimum_gear = int(fields["minimumGear"])
        maximum_gear = int(fields["maximumGear"])
        if shift_expected not in (0, 1):
            errors.append("automaticShiftExpected is not boolean")
        elif shift_expected:
            if gear_changes <= 0 or maximum_gear <= minimum_gear:
                errors.append("eligible automatic gearbox did not advance")
        elif gear_changes or maximum_gear != minimum_gear:
            errors.append("automatic gearbox shifted below its threshold")
    except (KeyError, ValueError):
        pass
    ...
    print(
        f"[VEHICLE_FOUR_WHEEL_DRIVE_RUN] name={spec.name} "
    ...
    for error in errors:
        print(
            "[VEHICLE_FOUR_WHEEL_DRIVE_RUN_ERROR] "
...
def compare_repeats(results: dict[str, dict[str, str]]) -> bool:
    ...
    for solver, execution in (
        ...
        print(
            "[VEHICLE_FOUR_WHEEL_DRIVE_REPEAT] "
...
def print_execution_comparison(
    ...
    try:
        ...
        values = (
            ...
            float(parallel["maxEngineSpeed"]),
            float(sequential["maxEngineSpeed"]),
        )
        valid = all(math.isfinite(value) for value in values)
    except (KeyError, ValueError) as exc:
        print(f"[VEHICLE_FOUR_WHEEL_DRIVE_EXECUTION_ERROR] error={exc}")
        return False
    print(
        "[VEHICLE_FOUR_WHEEL_DRIVE_EXECUTION] "
        f"parallelForward={values[0]:.9g} "
        f"sequentialForward={values[1]:.9g} "
        f"parallelHeading={values[2]:.9g} "
        f"sequentialHeading={values[3]:.9g} "
        f"parallelEngine={values[4]:.9g} "
        f"sequentialEngine={values[5]:.9g} "
...
def main() -> int:
    ...
    if not (bin_dir / EXECUTABLE).is_file():
        print(
            "[VEHICLE_FOUR_WHEEL_DRIVE_RUNNER_ERROR] "
    ...
    print(
        f"[VEHICLE_FOUR_WHEEL_DRIVE_MATRIX] mode={args.mode} "
