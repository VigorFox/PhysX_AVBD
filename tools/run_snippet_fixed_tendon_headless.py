#!/usr/bin/env python3
"""Run the SnippetFixedTendon failure-first or acceptance matrix headlessly."""
...
def specs() -> list[RunSpec]:
    result: list[RunSpec] = []
    for case_name in ("drive-a", "offset-actuation"):
        result.append(
            RunSpec(
                f"{case_name}-tgs",
                "tgs",
...
def run_one(
    ...
    print(
        f"[FIXED_TENDON_RUN] name={spec.name} "
    ...
    for error in errors:
        print(f"[FIXED_TENDON_RUN_ERROR] name={spec.name} error={error}")
...
def main() -> int:
    ...
    if not executable.is_file():
        print(f"[FIXED_TENDON_RUNNER_ERROR] missing executable: {executable}")
        return 2
    if args.timeout <= 0.0:
        print("[FIXED_TENDON_RUNNER_ERROR] --timeout must be positive")
        return 2
    ...
    print(
        f"[FIXED_TENDON_MATRIX] mode={args.mode} "
