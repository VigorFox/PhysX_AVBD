#!/usr/bin/env python3
"""Gate off-principal native revolute motor response headlessly."""
...
def run_one(spec: RunSpec, bin_dir: Path, timeout: float) -> bool:
    argv = [
        ...
        "--case=revolute-motor-off-principal",
    ...
    prefixes = {
        "gate": "[AVBD_GATE] ",
        "fixture": "[PROBE] [SnippetJointRevoluteMotorOffPrincipal] ",
        "cleanup": (
    ...
    if spec.expected_status == "FAIL" and gate.get("reason") not in {
        "revolute_motor_off_principal_velocity",
        "revolute_motor_off_principal_swing",
        "revolute_motor_off_principal_joint_error",
    }:
    ...
    if all(value is not None for value in metrics.values()):
        ...
        if spec.expected_status == "PASS" and not physical_pass:
            errors.append("PASS status lacks off-principal motor semantics")
        if spec.expected_status == "FAIL" and physical_pass:
            errors.append("FAIL status lacks an independent red metric")

    print(
        f"[REVOLUTE_MOTOR_OFF_PRINCIPAL_RUN] solver={spec.solver} "
        f"execution={spec.execution} "
        f"status={gate.get('status', 'MISSING')} "
        f"reason={gate.get('reason', 'MISSING')} "
        f"response={fixture.get('initialOffPrincipalResponse', 'MISSING')} "
        ...
        f"reaction={fixture.get('maximumSwingReaction', 'MISSING')} "
    ...
    for error in errors:
        print(
            f"[REVOLUTE_MOTOR_OFF_PRINCIPAL_ERROR] "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run off-principal native revolute motor headlessly."
    )
    parser.add_argument(
        "--mode",
    ...
    print(
        f"[REVOLUTE_MOTOR_OFF_PRINCIPAL_SUMMARY] mode={args.mode} "
