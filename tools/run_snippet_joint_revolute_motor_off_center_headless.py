#!/usr/bin/env python3
"""Gate off-center native revolute motor response headlessly."""
...
def run_one(spec: RunSpec, bin_dir: Path, timeout: float) -> bool:
    argv = [
        ...
        "--case=revolute-motor-off-center",
    ...
    if all(value is not None for value in metrics.values()):
        ...
        if spec.expected_status == "PASS" and not physical_pass:
            errors.append("PASS status lacks off-center motor semantics")
        if spec.expected_status == "FAIL" and physical_pass:
            errors.append("FAIL status lacks an independent red metric")

    print(
        f"[REVOLUTE_MOTOR_OFF_CENTER_RUN] solver={spec.solver} "
        f"execution={spec.execution} "
    ...
    for error in errors:
        print(
            f"[REVOLUTE_MOTOR_OFF_CENTER_ERROR] "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run off-center native revolute motor headlessly."
    )
    parser.add_argument(
        "--mode",
    ...
    print(
        f"[REVOLUTE_MOTOR_OFF_CENTER_SUMMARY] mode={args.mode} "
