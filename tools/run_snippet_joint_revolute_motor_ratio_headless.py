#!/usr/bin/env python3
"""Gate native revolute non-unit drive-ratio semantics headlessly."""
...
def run_one(spec: RunSpec, bin_dir: Path, timeout: float) -> bool:
    ...
    if all(value is not None for value in metrics.values()):
        physical_pass = (
            abs(metrics["finalWeightedVelocity"] - 2.0) <= 0.05
            and metrics["finalWeightedVelocityError"] <= 0.05
            and metrics["maximumLateWeightedVelocityError"] <= 0.05
            and metrics["maximumGeneralizedMomentumDrift"] <= 1.0e-3
        ...
        if spec.expected_status == "PASS" and not physical_pass:
            errors.append("PASS status lacks independent ratio semantics")
        if spec.expected_status == "FAIL" and physical_pass:
            errors.append("FAIL status lacks an independent red metric")

    print(
        f"[REVOLUTE_MOTOR_RATIO_RUN] solver={spec.solver} "
        f"execution={spec.execution} "
        f"status={gate.get('status', 'MISSING')} "
        f"reason={gate.get('reason', 'MISSING')} "
        f"velocityA={fixture.get('finalVelocityA', 'MISSING')} "
        f"velocityB={fixture.get('finalVelocityB', 'MISSING')} "
        f"weighted={fixture.get('finalWeightedVelocity', 'MISSING')} "
        f"weightedError="
    ...
    for error in errors:
        print(
            f"[REVOLUTE_MOTOR_RATIO_ERROR] solver={spec.solver} "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run native revolute non-unit drive ratio headlessly."
    )
    parser.add_argument(
        "--mode",
    ...
    print(
        f"[REVOLUTE_MOTOR_RATIO_SUMMARY] mode={args.mode} "
