#!/usr/bin/env python3
"""Run the SnippetCCD failure-first or acceptance matrix headlessly."""
...
def run_one(
    ...
    for error in errors:
        print(f"[CCD_RUN_ERROR] name={spec.name} error={error}")
...
def main() -> int:
    ...
    if not executable.is_file():
        print(f"[CCD_RUNNER_ERROR] missing executable: {executable}")
        return 2
    if args.timeout <= 0.0:
        print("[CCD_RUNNER_ERROR] --timeout must be positive")
        return 2
    ...
    print(
        f"[CCD_MATRIX] mode={args.mode} physicsPasses={physics_passes} "
