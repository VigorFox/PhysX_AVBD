#!/usr/bin/env python3
"""Run the SnippetSpatialTendon failure-first or acceptance matrix headlessly."""
...
def specs() -> list[RunSpec]:
    result: list[RunSpec] = []
    for case_name in ("offset-actuation", "attachment-actuation"):
        result.append(
...
def run_one(
    ...
    print(
        f"[SPATIAL_TENDON_RUN] name={spec.name} "
    ...
    for error in errors:
        print(f"[SPATIAL_TENDON_RUN_ERROR] name={spec.name} error={error}")
...
def main() -> int:
    ...
    if not executable.is_file():
        print(f"[SPATIAL_TENDON_RUNNER_ERROR] missing executable: {executable}")
        return 2
    if args.timeout <= 0.0:
        print("[SPATIAL_TENDON_RUNNER_ERROR] --timeout must be positive")
        return 2

    all_infrastructure_passed = True
    physics_passes = 0
    physics_failures = 0
    avbd_fingerprints: dict[str, str] = {}
    for spec in specs():
        passed, fields = run_one(spec, args.mode, bin_dir, args.timeout)
        if passed and spec.solver == "avbd":
            fingerprint = fields.get(
                "_jointObjectiveFingerprintSequence", ""
            )
            previous = avbd_fingerprints.setdefault(
                spec.case_name, fingerprint
            )
            if previous != fingerprint:
                print(
                    "[SPATIAL_TENDON_RUNNER_ERROR] "
                    f"case={spec.case_name} parallel/sequential "
                    "joint objective fingerprint mismatch"
                )
                passed = False
    ...
    print(
        f"[SPATIAL_TENDON_MATRIX] mode={args.mode} "
