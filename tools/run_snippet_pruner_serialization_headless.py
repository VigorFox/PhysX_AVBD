#!/usr/bin/env python3
"""Run the SnippetPrunerSerialization headless round-trip gate."""
...
EXECUTABLE = "SnippetPrunerSerialization_64.exe"
FRAMES = 240
...
def run_one(
    ...
    required = {
        "schema": "1",
        "snippet": "SnippetPrunerSerialization",
        "solver": spec.solver,
        "case": "pruner-roundtrip",
    ...
    try:
        if int(fields["serializedBytes"]) <= 0:
            errors.append(
                f"serializedBytes={fields['serializedBytes']}, expected > 0"
            )
    except (KeyError, ValueError):
        errors.append(
            f"serializedBytes={fields.get('serializedBytes')!r}, "
            "expected integer"
        )
    ...
    print(
        f"[PRUNER_SERIALIZATION_RUN] name={spec.name} "
    ...
    for error in errors:
        print(
            f"[PRUNER_SERIALIZATION_RUN_ERROR] "
...
def compare_repeats(results: dict[str, dict[str, str]]) -> bool:
    ...
    for solver, execution in (
        ...
        print(
            f"[PRUNER_SERIALIZATION_REPEAT] pair={solver}-{execution} "
...
def main() -> int:
    ...
    if not (bin_dir / EXECUTABLE).is_file():
        print(
            "[PRUNER_SERIALIZATION_RUNNER_ERROR] "
            f"missing executable: {bin_dir / EXECUTABLE}"
        )
        return 2
    if args.timeout <= 0:
        print("[PRUNER_SERIALIZATION_RUNNER_ERROR] --timeout must be positive")
        return 2
    ...
    print(
        f"[PRUNER_SERIALIZATION_MATRIX] mode={args.mode} "
