#!/usr/bin/env python3
"""Run SnippetCustomGeometryCollision through the hidden headless launcher."""
...
EXECUTABLE = "SnippetCustomGeometryCollision_64.exe"
CASES = ("falling", "sliding", "impact")
...
def positive_int(
    ...
    try:
        if int(fields.get(key, "0")) <= 0:
            errors.append(f"{key}={fields.get(key)!r}, expected positive")
    except ValueError:
...
def run_one(
    ...
    for key in (
        ...
        positive_int(fields, key, errors)
    ...
    print(
        f"[CUSTOM_GEOMETRY_COLLISION_RUN] name={spec.name} "
    ...
    for error in errors:
        print(
            "[CUSTOM_GEOMETRY_COLLISION_RUN_ERROR] "
...
def main() -> int:
    ...
    if not executable.is_file():
        print(
            "[CUSTOM_GEOMETRY_COLLISION_RUNNER_ERROR] "
    ...
    print(
        f"[CUSTOM_GEOMETRY_COLLISION_MATRIX] mode={args.mode} "
